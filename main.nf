...
/*
========================================================================================
                         nf-core/extractcoding
========================================================================================
 nf-core/extractcoding Analysis Pipeline.
 #### Homepage / Documentation
 https://github.com/nf-core/extractcoding
----------------------------------------------------------------------------------------
*/
...
def create_workflow_summary(summary) {
    ...
    yaml_file.text  = """
    id: 'nf-core-extractcoding-summary'
    description: " - this information is collected when the pipeline is started."
    section_name: 'nf-core/extractcoding Workflow Summary'
...
workflow.onComplete {
    ...
    def subject = "[nf-core/extractcoding] Successful: $workflow.runName"
    if (!workflow.success) {
      subject = "[nf-core/extractcoding] FAILED: $workflow.runName"
    }
    def email_fields = [:]
    ...
    try {
        if (workflow.success) {
            ...
            if (mqc_report.getClass() == ArrayList) {
                log.warn "[nf-core/extractcoding] Found multiple reports from process 'multiqc', will use only one"
                mqc_report = mqc_report[0]
            }
        }
    } catch (all) {
        log.warn "[nf-core/extractcoding] Could not attach MultiQC report to summary email"
    }
    ...
    if (email_address) {
        try {
          ...
          log.info "[nf-core/extractcoding] Sent summary e-mail to $email_address (sendmail)"
        } catch (all) {
          // Catch failures and try with plaintext
          [ 'mail', '-s', subject, email_address ].execute() << email_txt
          log.info "[nf-core/extractcoding] Sent summary e-mail to $email_address (mail)"
        }
    }
    ...
    if (workflow.success) {
        log.info "${c_purple}[nf-core/extractcoding]${c_green} Pipeline completed successfully${c_reset}"
    } else {
        checkHostname()
        log.info "${c_purple}[nf-core/extractcoding]${c_red} Pipeline completed with errors${c_reset}"
    }
...
def nfcoreHeader(){
    ...
    ${c_blue}  | \\| |       \\__, \\__/ |  \\ |___     ${c_green}\\`-._,-`-,${c_reset}
                                            ${c_green}`._,._,\'${c_reset}
    ${c_purple}  nf-core/extractcoding v${workflow.manifest.version}${c_reset}
    -${c_dim}--------------------------------------------------${c_reset}-
    """.stripIndent()
}
